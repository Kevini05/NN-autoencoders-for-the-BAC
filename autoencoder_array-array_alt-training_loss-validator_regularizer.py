...
def encoder_generator(N,k):
  ...
  x = Dense(units=128, activation=activation, kernel_initializer=initializer)(inputs_encoder)
  x = BatchNormalization()(x)
  x = Dense(units=64, activation=activation, kernel_initializer=initializer)(x)
  x = BatchNormalization()(x)
...
def decoder_generator(N,k):
  ...
  x = Dense(units=128, activation=activation, kernel_initializer=initializer)(inputs_decoder)
  x = BatchNormalization()(x)
  x = Dense(units=64, activation=activation, kernel_initializer=initializer)(x)
  x = BatchNormalization()(x)
  outputs_decoder = Dense(units=k, activation='sigmoid', kernel_initializer=initializer)(x)
...
def meta_model_generator(k,model_enc,model_dec,round,epsilon_t):
  ...
  # Build Model
  meta_model = keras.Model(inputs=inputs_meta, outputs=[encoded_bits,decoded_bits,decoded_bits_1,decoded_bits_2,decoded_bits_3,decoded_bits_4],name = 'meta_model')
  return meta_model
...
if pretraining:
  ...
  ### Fit the model
  history = meta_model.fit(U_k, [aux,U_k,U_k,U_k,U_k,U_k], epochs=epoch_pretrain, verbose=verbose, shuffle=True, batch_size=batch_size)
...
for a in range(iterations + 1):
  ...
  ### Fit the model
  history = meta_model.fit(U_k, [aux,U_k, U_k, U_k, U_k, U_k], epochs=epoch_int, verbose=verbose, shuffle=True, batch_size=batch_size, callbacks=cbks)
...
title = f'N={N} k={k} {length_training} - NN Array_array_Iterative_training_regularizer'
...
# \Python3\python.exe autoencoder_array-array_alt-training_loss-validator_regularizer.py 4 2 2 1000 medium
